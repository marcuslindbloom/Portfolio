...
import { ANIMATIONS, DIRECTIONS, PLAYER, ENTITIES, INPUT, ANGLES, scaleFactor, SPRITES } from "../constants";
...
export const setupControls = (player: GameObj) => {
    ...
    kbm.onMouseDown((mouseBtn: MouseButton) => {
        ...
        if (mouseAngle > ANGLES.LOWER_BOUND && mouseAngle < ANGLES.UPPER_BOUND && player.curAnim() !== ANIMATIONS.WALK_UP) {
            player.play(ANIMATIONS.WALK_UP);
            player.direction = DIRECTIONS.UP;
            return;
        }

        if (mouseAngle < -ANGLES.LOWER_BOUND && mouseAngle > -ANGLES.UPPER_BOUND && player.curAnim() !== ANIMATIONS.WALK_DOWN) {
        ...
        if (Math.abs(mouseAngle) > ANGLES.UPPER_BOUND) {
            player.flipX = false;
            if (player.curAnim() !== ANIMATIONS.WALK_SIDE) player.play(ANIMATIONS.WALK_SIDE);
    ...
    kbm.onKeyDown(() => {
        ...
        if (keyMap.up) {
            if (player.curAnim() !== ANIMATIONS.WALK_UP) player.play(ANIMATIONS.WALK_UP);
            player.direction = DIRECTIONS.UP;
        ...
        if (keyMap.down) {
            if (player.curAnim() !== ANIMATIONS.WALK_DOWN) player.play(ANIMATIONS.WALK_DOWN);
            player.direction = DIRECTIONS.DOWN;
            player.move(0, player.speed);
        }
    });
